...
#         return list(action + self.noise.sample())  # add some noise for exploration
        noisy_action = list(action + self.noise.sample())  # add some noise for exploration
#         print("DEBUG: noisy_action: {}\n".format(noisy_action))
        # ensure we are within the bounds:
        result = []
        for action in noisy_action:
            if action <  self.action_low:
                action = self.action_low
            elif action > self.action_high:
